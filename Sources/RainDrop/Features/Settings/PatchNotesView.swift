...
private let patchNotes: [PatchNote] = [
        ...
        PatchNote(version: "2.8.3", date: "2026.04.12", changes: [
            ...
            "물 출렁임 애니메이션 복원 — 타이머 실행 중 파도 정상 동작",
            "무한 모드 물 채움 고착 버그 수정 — 사이클 반복 후에도 정상 채움",
        ]),
        PatchNote(version: "2.8.2", date: "2026.04.12", changes: [
            ...
            "물 채움 애니메이션 안정성 대폭 개선 — 마스크 바닥 정렬 수정",
            "스킨별 물 바닥 위치를 실제 양동이 형태에 정밀 맞춤",
        ]),
        PatchNote(version: "2.8.1", date: "2026.04.12", changes: [
            "물 색상 선택 안내 문구 추가 (금 양동이부터 활성화)",
            "물 슬로싱 제거 — 파도 애니메이션 끊김 해소",
            "양동이 탭 흔들림 복원 (물 물리와 분리)",
            "테라리움 식물 배치 영구 저장 (재시작해도 동일 배치 유지)",
            "설정 탭 레이블 '환경' → '스킨'으로 변경",
            "양동이 흔들림 타이밍 개선 (스프링 애니메이션 자연스러움 향상)",
        ]),
