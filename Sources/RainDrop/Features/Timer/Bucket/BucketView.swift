    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            TimelineView(.animation) { context in
                let waveOffset = mode == .full
                    ? ((context.date.timeIntervalSinceReferenceDate / Self.waveCycleDuration)
                ...
                ZStack(alignment: .bottom) {
                    ...
                    // 9. Handle — removed for cleaner design
                }
...
/// Wraps provider.handlePath as a SwiftUI Shape
@MainActor
private struct BucketHandleShapeWrapper: Shape {
    let provider: AnyBucketSkin

    nonisolated func path(in rect: CGRect) -> Path {
        MainActor.assumeIsolated {
            provider.handlePath(in: rect)
        }
    }
}
